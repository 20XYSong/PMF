...
class PMF:
    ...
    def SGD_update(self,train):
        ...
        for t in range(self.iter_max):
            ...
            rmse=self.rmse
            rmse_vec_sgd[t+1]=rmse

            # if abs(obj-obj_last)<acc:
            #     self.real_iter = t + 1
            #     break
...
if __name__ == '__main__':
    latent_fac_num,stepsize,sigma2,sigmau,sigmav,itermax=5,0.01,0.001,0.01,0.01,10000
    # time_s=time.time()
    ...
    plt.plot(iter_vec, pmf.rmse_vec, label='rmse({})'.format(time_sgd), color='blue', linewidth=0.8)
    plt.show()
